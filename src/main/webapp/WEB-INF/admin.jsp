		...
		<table id="myTable" class="table table-striped align-middle border rounded overflow-hidden bg-gradient shadow">
			<thead>
				<tr>
					...
					<th class="custom-bg-color text-dark">Status</th>	
			...
			<tbody class="table-group-divider">
				<c:forEach var="oneUser" items="${allUsers}">
					<tr>
						...
						<c:choose>
							<c:when test="${oneUser.getStatus().getState().contains('STATUS_BANNED')}">
								<td style="color:red;">Banned</td>
							</c:when>
							<c:when test="${oneUser.getStatus().getState().contains('STATUS_INACTIVE')}">
								<td style="color:darkorange;">Inactive</td>
							</c:when>
							<c:otherwise>
						...
						<c:choose>
							...
							<c:otherwise>
								<td>
									<c:choose>
										<c:when test="${oneUser.getStatus().getState().contains('STATUS_BANNED')}">
											<a href="/users/activate/${oneUser.getId()}"><button class="btn btn-sm btn-warning my-1">Unban</button></a>
										</c:when>
										<c:when test="${oneUser.getStatus().getState().contains('STATUS_INACTIVE')}">
											<a href="/users/activate/${oneUser.getId()}"><button class="btn btn-sm btn-success my-1">Activate</button></a>
										</c:when>
										<c:otherwise>
